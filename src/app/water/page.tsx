              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {topMismatchCounties.length ? topMismatchCounties.map((county) => (
                  <Link
                    ...
                    href={`/water?county=${county.county.slug}`}
          ...
          {breakdown ? (
            <div className="mt-6 space-y-5">
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-4">
                  <div className="text-sm text-slate-400">Impaired surface-water segments</div>
                  <div className="mt-2 text-3xl font-semibold text-white">{formatNumber(breakdown.county.metrics.impairedSurfaceWaterSegmentCount)}</div>
                </div>
                <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-4">
                  <div className="text-sm text-slate-400">Mismatch score</div>
                  <div className="mt-2 text-3xl font-semibold text-white">{formatNumber(breakdown.county.mismatch?.score)}</div>
                </div>
              </div>
              <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-4 text-sm text-slate-300">
                Governance entities: {breakdown.layers.governance.length} · Alerts: {breakdown.layers.alerts.length} · Gauges: {breakdown.layers.gauges.length} · Sewer overflows: {breakdown.layers.sewerOverflows.length} · Permits: {breakdown.layers.permits.length} · Surface-water segments: {breakdown.layers.surfaceWaterQuality?.length ?? 0}
              </div>
              {breakdown.county.mismatch?.flags?.length ? (
                <div className="rounded-2xl border border-amber-700/40 bg-amber-950/30 p-4 text-sm text-amber-100">
            ...
            <table className="min-w-full divide-y divide-slate-800 text-sm">
              <thead>
                <tr className="text-left text-slate-400">
                  ...
                  <th className="px-3 py-3 font-medium">Mismatch</th>
                </tr>
              </thead>
