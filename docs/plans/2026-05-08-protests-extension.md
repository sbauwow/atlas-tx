...
- **Search One live fragility.** As of 2026-05-08 scripted Search One POSTs still return the upstream error page even for county/program chunks. The executable refresh scaffold exists and now supports a browser-automation fallback hook, but unattended statewide case refresh is not yet reliable until that fallback is implemented against a real browser retriever.
