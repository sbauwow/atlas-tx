...
| cross (web→data) | hermes | Back /permits with a true Texas county choropleth using county polygons from `us-atlas`, permit-count color bands, and CID hatch overlays. Replaced centroid tiles with real county shapes, extracted `texas-county-choropleth.tsx`, and covered the new map path rendering in tests. | working tree |
