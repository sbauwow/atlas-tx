...
| cross (web→data) | hermes | Back /permits with a Texas county permit map using county-centroid tiles, permit-count color bands, and CID hatch overlays. Added county map row builder in `tceq-permits.ts`, page rendering, and tests. | working tree |
